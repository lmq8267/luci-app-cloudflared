...
.modal-content { background-color: #fff; margin: 10% auto; padding: 20px; border-radius: 6px; width: 80%; max-width: 700px; word-wrap: break-word; overflow-wrap: break-word; white-space: pre-wrap; max-height: 70vh; }
.modal-close { float: right; font-size: 20px; cursor: pointer; }
#topBtn { display: none; position: fixed; bottom: 100px; right: 20px; z-index: 1000; font-size: 14px; padding: 8px 12px; border: none; border-radius: 4px; background-color: #007aff; color: #fff; cursor: pointer; }
</style>
...
<div class="controls">
  ...
  <div class="log-count">共 ]] .. #log_table .. [[ 条日志</div>
	...
	for i, logobj in ipairs(log_table) do
		...
		http.write("<div id='modal_" .. i .. "' class='modal'><div class='modal-content'><span class='modal-close' onclick='closeModal(" .. i .. ")'>&times;</span><pre style='overflow-x: hidden; white-space: pre-wrap; word-wrap: break-word;'>" .. json_str .. "</pre></div></div>")
	end
...
function clearLogs(){
	window.location.href = "?form=logview&_clear=1";
}

function refreshLogs(){
	window.location.href = "?form=logview&_refresh=1";
}
</script>
